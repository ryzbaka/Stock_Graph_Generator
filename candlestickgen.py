...
def gen_sp_candlestick(ticker,start=dt.datetime(dt.datetime.now().year-5,1,1),stop=dt.datetime(dt.datetime.now().year,dt.datetime.now().month,dt.datetime.now().day),refresh_data=False):
    ...
    ax3=plt.subplot2grid((6,2),(0,1),rowspan=6,colspan=1)
    candlestick_ohlc(ax1,resampled.values,width=2,colorup='g')
    ax2.plot(df[f'{ticker}_Volume'],color='blue')
    ax3.plot(df[f'{ticker}_Open'])
    ax3.plot(df[f'{ticker}_High'])
    ax3.plot(df[f'{ticker}_Low'])
    ax3.plot(df[f'{ticker}_Close'])
    ax3.plot(df[f'{ticker}_Adj Close'].rolling(50).mean(),color='red')
    plt.title(f'{ticker}')
    plt.show()
